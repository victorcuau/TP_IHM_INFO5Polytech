...
/**
 * 
 * On étend la classe BasicSliderUI pour y ajouter un deuxième levier permettant
 * d'ajuster la borne supérieure de notre fourchette. Les modifications ne sont
 * pas très compliquées, il suffit de créer un nouveau rectangle qui représente
 * la borne droite, puis de modifier les fonctions existantes afin que les mises
 * à jour lors des modifications soient bien prises en compte dans le model
 * ainsi que graphiquement. On rajoute aussi des contraintes liées au
 * fonctionnement d'un range slider. La machine à état se trouve donc ici dans
 * le rangeTrackListener.
 */
...
public class RangeSliderUI extends BasicSliderUI {
	...
	private transient boolean isDraggingIntervalle;
	private transient int secondOffset;
	private transient int saveOffset;
	...
	// On ne souhaite pas que l'ancien code d'affichage des bornes soit utilisé, on
	// utilise le notre.
	...
	public void paintThumbRight(Graphics g) {
		...
		// Permet de changer la forme de la borne, on essaie de se rapprocher d'un
		// design minimaliste souvent retrouvé sur les sites web
	...
	public void setRightThumbLocation(int x, int y) {
		...
		SwingUtilities.computeUnion(newThumbRect.x, newThumbRect.y, newThumbRect.width, newThumbRect.height,
				rightUnion);
	...
	public class RangeTrackListener extends TrackListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (!rightPressed && !leftPressed) {
				if (currentMouseX < thumbRect.x) {
					...
					mouseDragged(e);
					return;
				} else if (currentMouseX > (newThumbRect.x + newThumbRect.width)) {
					isDraggingRight = true;
					offset = thumbRect.width / 2;
					mouseDragged(e);
					return;
				} else if((currentMouseX > (thumbRect.x + thumbRect.width)) && currentMouseX < newThumbRect.x) {
					offset = currentMouseX - thumbRect.x;
					secondOffset = currentMouseX - newThumbRect.x;
					isDraggingIntervalle = true;
					return;
				}
			}
			isDraggingLeft = false;
		...
		public void mouseClicked(MouseEvent e) {
			...
			if ((currentMouseX > (thumbRect.x + thumbRect.width)) && currentMouseX < newThumbRect.x) {
				int moyenne = (thumbRect.x + newThumbRect.x + thumbRect.width )/2;
				if (currentMouseX < moyenne) {
					isDraggingLeft = true;
					offset = thumbRect.width / 2;
					mouseDragged(e);
				} else {
		...
		public void mouseDragged(MouseEvent e) {
			...
			} else if (isDraggingIntervalle) {
				slider.setValueIsAdjusting(true);
				mouseDraggedIntervalle();
			}
		}
		
		private void mouseDraggedIntervalle() {
			mouseDraggedLeftThumb();
			saveOffset = offset;
			offset = secondOffset;
			mouseDraggedRightThumb();
